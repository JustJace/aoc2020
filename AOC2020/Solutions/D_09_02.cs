...
{
    public class D_09_2 : Solver<long>
    {
        ...
        protected override long GetAnswer(string input)
        {
            ...
            var weakness = FindMissingSumWeakness(numbers);

            for (var i = 0; i < numbers.Length - 1; i++)
            for (var j = i + 1; j < numbers.Length; j++)
            if (Sum(i, j, numbers) == weakness)
                return Min(i, j, numbers) + Max(i, j, numbers);

            throw new Exception("couldn't find answer");
        }
        private long Min(int low, int high, long[] numbers) => numbers.Skip(low).Take(high - low).Min();
        private long Max(int low, int high, long[] numbers) => numbers.Skip(low).Take(high - low).Max();
        private long Sum(int low, int high, long[] numbers) => numbers.Skip(low).Take(high - low).Sum();
